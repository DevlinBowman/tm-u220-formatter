...
test("headerless input interprets directives while preserving heading lines", function()
    local result = compiler.compile_input("# heading\n@cut installed\nBody")
    check.equal(#result.diagnostics, 0)
    check.truthy(result.bytes)
...
test("a readable ordinary document is interpreted without losing headings", function()
    with_file(".txt", "# Heading\n@cut installed\nordinary text\n", function(path)
        local resolved, failure = resolver.resolve(path)
        check.falsy(failure)
        check.equal(resolved.input_kind, "interpreted")
        check.equal(resolved.path, path)
        check.equal(resolved.profile_path, defaults.profile_path())
        check.contains(resolved.source, "## Heading\n@cut installed\nordinary text")
    end)
end)
...
test("ordinary documents compile with headings and active directives", function()
    with_file(".txt", "# Heading\n@cut installed\nBody\n", function(path)
